...
class DogList extends Component {
  ...
  componentDidMount() {
    axios.get('https://dog.ceo/api/breed/pointer/images').then(res => {
  ...
  render() {
    ...
    const favMap = favs.map((e, i) => {
      return (
        <Dogs
          key={i}
          img={e}
          handleClick={e => this.handleDelete(i)}
          button={'Delete'}
        />
      );
    });
    return (
      <div className="dog">
        ...
        <div className="dog_map map1">{map}</div>
        <h2>Favorite Puppers</h2>
        <div className="dog_map map2">{favMap}</div>
        <StarWars />
      </div>
    );
  }
